...
import { usageFeatureLabels, usageFeatureLabel, usageSystemLabel, usageMetricText, usageDurationText as hours } from "@/lib/usagePresentation";
...
const eventLabels: Record<string, string> = {
  ...
  custom_system_saved: "系统配置操作结果", agent_task_result: "Agent 操作结果", usage_session_summary: "运行与使用摘要",
  error_reported: "错误上报",
};
const resultLabels: Record<string, string> = { success: "成功", failure: "失败", cancelled: "已取消", unknown: "未知" };
type Filters = { from: string; to: string; environment: "production" | "development" | "test" | "all"; customerKey?: string; appVersion?: string; module?: string; featureId?: string };
...
export default function UsageAnalytics() {
  ...
  return <div className="min-w-0 space-y-6 p-4 md:p-6">
    ...
    <form className="grid gap-4 rounded-lg border bg-card p-4 sm:grid-cols-2 xl:grid-cols-4" onSubmit={event => { event.preventDefault(); if (valid) apply(draft); }}>
      ...
      <label className="space-y-1.5 text-sm"><span>功能或运行指标</span><select className={selectClass} value={draft.featureId ?? ""} onChange={event => setDraft({ ...draft, featureId: event.target.value || undefined })}>
        <option value="">全部功能与指标</option>{Object.entries(usageFeatureLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
      </select></label>
    ...
    {overview.isError ? <div role="alert" className="rounded-lg border border-destructive p-5"><p className="font-medium">使用数据加载失败</p><p className="mt-1 text-sm">请稍后重试；尚未部署接收服务或数据库迁移时也会出现此提示。</p></div>
      : overview.isPending ? <p role="status" className="py-10 text-center text-muted-foreground">正在读取客户使用数据…</p> : data && <>
        ...
        {tab === "overview" ? <div className="space-y-6">
          <section className="min-w-0 rounded-lg border bg-card p-4"><h2 className="text-lg font-medium">功能使用排行</h2><p className="mt-1 text-xs text-muted-foreground">按事件类型和功能分开统计，展示前 {data.listLimit} 项。入口展示、尝试和完成分别计数。</p>
            <div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><caption className="sr-only">功能使用数量、客户覆盖和结果</caption><thead><tr className="border-b text-muted-foreground">{["功能 / 事件", "客户数", "事件数", "成功 / 失败", "有数据天数"].map(title => <th key={title} scope="col" className="whitespace-nowrap px-3 py-3 font-medium">{title}</th>)}</tr></thead>
              <tbody>{data.features.map(row => <tr key={`${row.eventName}:${row.featureId}`} className="border-b last:border-0"><td className="px-3 py-3"><span>{eventLabels[row.eventName] ?? row.eventName}</span>{row.featureId && <span className="mt-1 block text-xs text-muted-foreground">{usageFeatureLabel(row.featureId)}</span>}</td><td className="px-3 py-3 tabular-nums">{row.customers}</td><td className="px-3 py-3 tabular-nums">{row.events}</td><td className="px-3 py-3 tabular-nums">{row.successes} / {row.failures}</td><td className="px-3 py-3 tabular-nums">{row.activeDays}</td></tr>)}</tbody></table></div>
          </section>
          <section className="rounded-lg border bg-card p-4"><h2 className="text-lg font-medium">展示系统使用</h2>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">展示前 {data.listLimit} 项。匿名系统号只在同一授权、同一安装内稳定，不代表公司内所有电脑上的同一个系统；不上传客户自定义名称。</p>
            <div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><caption className="sr-only">系统进入次数与数据流活跃时间</caption><thead><tr className="border-b text-muted-foreground">{["系统", "客户 / 安装", "进入次数", "监测时长"].map(label => <th key={label} scope="col" className="whitespace-nowrap px-3 py-3">{label}</th>)}</tr></thead>
              <tbody>{data.systems?.map(row => <tr key={row.systemType} className="border-b last:border-0"><td className="min-w-56 break-words px-3 py-3">{usageSystemLabel(row.systemType)}</td><td className="px-3 py-3">{row.customers} / {row.installations}</td><td className="px-3 py-3">{row.entries}</td><td className="whitespace-nowrap px-3 py-3">{hours(row.monitoringMs)}</td></tr>)}</tbody></table></div>
            {!data.systems?.length && <p className="py-4 text-sm text-muted-foreground">此范围内尚未收到带系统分类的事件。</p>}
          </section>
          <section className="rounded-lg border bg-card p-4"><h2 className="text-lg font-medium">客户使用情况</h2><p className="mt-1 text-xs text-muted-foreground">按记录数展示前 {data.listLimit} 家。点击客户查看它的功能及操作时间线。</p><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><caption className="sr-only">客户安装数、使用天数、监测采集时长及错误</caption><thead><tr className="border-b text-muted-foreground">{["客户", "安装 / 天数", "监测 / 采集", "前台 / 交互窗口", "错误次数", "最近使用"].map(title => <th key={title} scope="col" className="whitespace-nowrap px-3 py-3 font-medium">{title}</th>)}</tr></thead><tbody>{data.customers.map(row => <tr key={row.customerKey} className="border-b last:border-0"><td className="min-w-44 px-3 py-2"><Button variant="link" className="h-auto min-h-11 max-w-64 whitespace-normal px-0 text-left" onClick={() => apply({ ...filters, customerKey: row.customerKey })}><span>{row.customerName}</span></Button></td><td className="px-3 py-3">{row.installations} / {row.activeDays}</td><td className="whitespace-nowrap px-3 py-3">{hours(row.monitoringMs)} / {hours(row.collectionMs)}</td><td className="whitespace-nowrap px-3 py-3">{hours(row.foregroundMs ?? 0)} / {hours(row.interactionMs ?? 0)}</td><td className="px-3 py-3">{row.errors}</td><td className="whitespace-nowrap px-3 py-3">{time(row.lastOccurredMs)}</td></tr>)}</tbody></table></div></section>
          ...
          {events.isError ? <p role="alert" className="text-destructive">记录加载失败，请点击刷新重试。</p> : events.isPending ? <p role="status">正在加载记录…</p> : !events.data?.items.length ? <p className="rounded border border-dashed p-6 text-muted-foreground">没有符合筛选条件的记录。</p> : events.data.items.map(row => <article key={`${row.sourceId}:${row.installationId}:${row.eventId}`} className="min-w-0 rounded-lg border bg-card p-4">
            ...
            <p className="mt-2 break-words text-sm text-muted-foreground">{[
              ...
              row.event.eventName !== "error_reported" && usageMetricText(row.event.properties),
              row.event.properties.action && `动作：${row.event.properties.action}`,
        ...
        <p className="text-xs leading-6 text-muted-foreground">客户名称按首次接收时的授权归属保留；软件换密钥后的来源单独记录。安装数按授权来源与安装标识组合计数。同一公司多份授权可能对应同一物理设备。前台表示窗口可见且有焦点；交互窗口指操作后 30 秒内的活跃区间，不是连续工作时长。性能记录是采样摘要，不是硬件故障结论。当前统计不推断未授权功能，也不把未上传算作未使用。</p>
      </>}
  </div>;
}
