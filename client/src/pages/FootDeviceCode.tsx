...
// ===== 脚垫插槽定义 =====
type FootSlot = "foot1" | "foot2" | "foot3" | "foot4";
...
const FOOT_SLOTS: FootSlot[] = ["foot1", "foot2", "foot3", "foot4"];

const SLOT_LABELS: Record<FootSlot, string> = {
  foot1: "脚垫 1",
  foot2: "脚垫 2",
  foot3: "脚垫 3",
  foot4: "脚垫 4",
...
const SLOT_COLORS: Record<FootSlot, { bg: string; border: string; text: string; ring: string }> = {
  foot1: {
    bg: "bg-blue-50",
      ...
      <CardContent className="p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            >
              <Footprints
                className={`h-5 w-5 ${isDone ? colors.text : "text-muted-foreground"}`}
              />
            </div>
            <div>
...
// ===== 主页面 =====
export default function FootDeviceCode() {
  const [slots, setSlots] = useState<Record<FootSlot, SlotState>>({
    foot1: { label: "脚垫 1", mac: null, status: "empty" },
    foot2: { label: "脚垫 2", mac: null, status: "empty" },
    foot3: { label: "脚垫 3", mac: null, status: "empty" },
    foot4: { label: "脚垫 4", mac: null, status: "empty" },
  });
  const [activeSlot, setActiveSlot] = useState<FootSlot | null>(null);
  ...
  const recordRead = (slot: FootSlot, mac: string | null, success: boolean) => {
  ...
  useEffect(() => {
    ...
    serialMacService.setOnMac((result) => {
      // 通过 ref 回调将 MAC 结果传递给当前活跃的 slot
  ...
  const handleReadSlot = useCallback(
    async (slot: FootSlot) => {
      ...
      // 自动探测连接（脚垫默认 3000000 波特率）
      const success = await serialMacService.autoDetectConnect();
      if (success) {
  ...
  const outputString = useMemo(() => {
    const parts = FOOT_SLOTS.map((slot) => {
  ...
  // 已完成的数量
  const doneCount = FOOT_SLOTS.filter((s) => slots[s].status === "done").length;
  const allDone = doneCount === 4;

  // 复制输出
  const handleCopy = useCallback(async () => {
    if (!outputString) {
      toast.error("请先读取至少一个脚垫的 MAC 地址");
      return;
    }
    try {
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-blue-50/50 border-blue-200/50">
        <CardContent className="py-3 px-4">
          <div className="flex items-center gap-6 text-sm text-blue-700">
            <div className="flex items-center gap-2">
              <span className="h-6 w-6 rounded-full bg-blue-100 flex items-center justify-center text-xs font-bold">
                1
              </span>
              <span>连接脚垫设备</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          {/* 输出结果 */}
          <Card
            className={`${allDone ? "border-emerald-300 bg-emerald-50/30" : "border-border/50"}`}
          >
            ...
            <CardContent className="space-y-3">
              ...
              >
                {outputString || "读取脚垫 MAC 地址后，设备码将在此显示..."}
              </pre>
              ...
              {allDone && (
                <div className="flex items-center gap-2 bg-emerald-100 border border-emerald-200 rounded-lg px-3 py-2">
                  <Check className="h-4 w-4 text-emerald-600 shrink-0" />
                  <span className="text-sm text-emerald-700 font-medium">
                    全部 4 个脚垫 MAC 地址已读取完成
                  </span>
                </div>
              )}
            </CardContent>
