...
实现日期：2026-09-24。服务器首版已观察到线上接收路由响应和包含菜单的静态资源；这不等同于真实客户数据验收。本轮扩展仍在隔离工作区，未发布或操作生产数据库。
...
- 前台与交互窗口只累加 `usage_session_summary` 的 `foreground` / `interaction`。前台要求窗口可见且有焦点；交互窗口为操作后 30 秒内的区间，不代表员工工作时长。
- `overview.systems` 按 properties.systemType 分组前 100 项，分别显示客户/安装数、真实进入次数、监测时长。`custom/copy.<16位摘要>` 由客户端按安装和授权隔离，不能跨安装匹配原始自定义系统，页面不显示客户系统名称。
- 新增可选 `featureId` 精确筛选；它只影响管理查询，不改变上传事件白名单。主进程内存 count 的单位为 MiB；事件循环延迟 durationMs 为最大延迟，长任务 durationMs 为累计时长，帧间隔 count 为超过阈值的间隔次数。时间线按类别显示单位，不把这些值当作监测或采集时长。
...
本轮沿用 schemaVersion 1 和既有事件名/属性，不新增迁移。已经部署首版的环境只需按发布流程更新本轮查询和页面，不重复执行 `0015`。旧客户端缺少细分时长时显示 0，旧 custom/copy 类别显示“未区分”；不能凭缺少事件判断用户没有使用。
...
- 本轮增强：完整 17 文件、159 项测试通过，包含独立临时 MySQL 中的 6 项 usage 集成测试和原聊天同步 10 项集成测试；新增匿名系统分组、前台/交互/监测时长隔离、功能筛选及指标单位展示回归。TypeScript 和临时目录生产构建通过，浏览器覆盖系统表格、功能筛选、1440/375、浅深色与低权限。未触及生产数据库或上传真实客户数据。
